...
\begin{response}
We now provide a more thorough comparison between PoolParty and other available tools for designing sequence libraries, including in the Introduction, Discussion, and a new Supplemental Information document. Specifically, a literature search identified 13 tools with significant use-case overlap with PoolParty; these are now listed in Table S2. Table S3 compares the specific features of PoolParty and the 7 most closely related tools. Figs.~S3 and S4 provide explicit use-case comparisons of PoolParty to the two tools we judged to be the closest competitors (VaLiAnT and tangermeme, respectively);  each comparison shows side-by-side code/commands that generates an identical library in PoolParty and in the competing tool. We also attempted a similar comparison with MPRAnator, but that tool (which runs through a web interface) is no longer functional. 

We have also benchmarked the compute and memory requirements of PoolParty on the three examples featured in the main text (Fig.~S2 and Table S1). Resource usage is shown only for PoolParty; no competing tool is able to generate the libraries in Figs.~2 and 3, and in any case the text makes no claims about PoolParty being faster or more memory efficient than other tools.
\end{response}
