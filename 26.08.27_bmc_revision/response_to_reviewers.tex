...
\begin{response}
We thank the editor and the three reviewers for their consideration of our
manuscript. This work has been greatly improved based on constructive feedback
from all reviewers. Additional edits have been made throughout the manuscript
to improve clarity. Below is a point-by-point response to each of the critiques
raised.
\end{response}
...
\begin{response}
We have addressed these points throughout the revision. We benchmarked
PoolParty runtime and memory usage for all three examples, each at a range of
library sizes. For tool comparisons, we included the ones discussed in the
manuscript that address DNA library design or related sequence-design problems.
We have now included two supplementary tables, S2 and S3, to give an overview
of these tools and compare detailed design capabilities for a subset focused on
DNA library design. For the code comparisons, we selected tools that could
cover the use case types represented by the examples in our main text. These
direct comparisons focus on code and generated sequences: VaLiAnT for DMS and
tangermeme for the SpliceAI surrogate modeling example. We also tried
MPRAnator for the MPRA example, but it could not be run at the time of testing.
We have made substantial changes to the Background and Discussion sections to
clarify the scope and limitations of PoolParty. Detailed responses are provided
below.
\end{response}
...
operations used. We have added these results to Additional file 1: Fig.~S2 and
Table~S1, with
details of the benchmarking setup in the Supplementary Methods. We have added the
...
\begin{response}
Most design cards record sequence changes introduced by PoolParty Operations as
human-readable descriptors, which do not add additional information since they
are fully recoverable from the final sequence. The main purpose of design cards
is thus to save users time and effort extracting those features post-hoc from
the constructed library using custom scripts, and to provide interpretable
inputs for downstream grouping and modeling when the recorded sequence changes
are themselves the object of study. In the SpliceAI surrogate example, we use
SpliceAI to score an in silico sequence library and fit a simple surrogate model
to those predictions. We are mainly interested in what SpliceAI has learned
about the effects of cryptic splice site strength and position on the usage of
a nearby canonical 5$'$ splice site, rather than in improving predictive
performance. Since the surrogate model is intrinsically interpretable and the
design cards directly encode cryptic site strength and relative position, its
fitted effects have a direct biological interpretation and show how SpliceAI
predictions respond to these two factors. These effects would be harder to
interpret from raw sequence inputs alone.
\end{response}
...
\begin{response}
We agree that a careful comparison with related tools is important for
clarifying the scope of PoolParty. We have now added two supplementary tables
comparing PoolParty with tools mentioned in the main text (Additional file 1:
Tables~S2 and S3). The first gives a general
overview of the tools and their key features, while the second one is a feature
matrix that focuses on specific
library-design capabilities. The comparison also includes features that
PoolParty does not currently support, such as those related to the physical
construction of the library and annotation-aware design.

We also compared PoolParty directly with VaLiAnT and tangermeme. For VaLiAnT,
we implemented a new BRCA1 exon 2 DMS library in both tools, and all 357
sequences matched when both used the same codon usage table. For tangermeme, we
used the SpliceAI example from the main text, and all 200,000 sequences matched
after converting the one-hot output from tangermeme back to sequence strings.
These comparisons are shown in Additional file 1: Figs.~S3 and S4. We also
tried to include MPRAnator, but it could not be run at the time of testing.

The supplementary figures show the implementations side by side. The
PoolParty and comparison implementations are comparable in length in both
examples; however, our main goal is to make complex library designs more
modular and declarative so that they are easier to specify and reproduce.
\end{response}
...
\begin{response}
We have addressed this together with Comment 1 above.
\end{response}

\item Include a table comparing features with 2--3 existing tools.

\begin{response}
The comparison is described in our response to Comment 3 above and provided in
...
\begin{response}
We thank the reviewer for these suggestions; however, having read these papers,
it remains unclear to us how they are relevant to PoolParty. Specifically, the
first reviews multimodal representations of small molecules for drug discovery;
the second models single-cell transcriptomic and surface-protein measurements;
the third reviews read mapping to pangenome graphs; and the last one is about
circRNA--miRNA interaction prediction. None seems to relate to designed oligo
pools; we have therefore decided not to include these citations.
\end{response}
...
\item The authors disclose that LLMs were used to develop, debug, and document
the code. Did the authors also write a test harness, and what edge cases were
nominated by the LLMs for testing vs what edge cases were nominated by the human
...
produce sequence pools as expected. Metrics such as the number of pool elements
that overlap and an investigation of any unique pool elements would ensure that
the generate pools are accurate.

\begin{response}
We discuss this comparison in our response to Reviewer 1, Comment 3. The
sequence-set comparisons are shown in Additional file 1: Figs.~S3 and S4.
\end{response}

\item additional operations to improve library content would be useful. E.g. to
maximize hamming distance, remove sequences containing restriction enzyme
sites, remove homopolymers, etc. would make the tool more useful in generating
ready-to-use pools.
...
\begin{msquote}
Filtering can be based on sequence length, GC content, homopolymer length,
sequence complexity, restriction enzyme recognition sites, or arbitrary
...
\begin{msquote}
PoolParty is not a sequence optimization tool, e.g., for optimizing codon usage
\citep{Swainston2017rb} or individual sequences subject to synthesis constraints
\citep{Zulkower2020jk}, or for designing sequences guided by machine learning
models \citep{Schreiber2025ledidi}.
\end{msquote}
...
\begin{response}
We agree that the original wording was too broad. We have removed language
suggesting that PoolParty replaces other tools and revised the Background and
Discussion to define its scope more clearly.

In Additional file 1, we have added two tables explicitly
comparing PoolParty with existing tools (Tables~S2 and S3). We have also added two
figures showing side-by-side implementations of the same libraries in
PoolParty and VaLiAnT or tangermeme (Figs.~S3 and S4).

PoolParty does not currently accept HGVS notation. We discuss the broader
genome- and transcript-awareness limitation under Comment 3 below. We have
revised the relevant statements as follows:
\end{response}
...
\begin{msquote}
We have presented PoolParty, a Python package that provides a structured
alternative to the ad hoc scripting commonly used to design oligonucleotide
libraries.
\end{msquote}
...
\begin{response}
We agree and have updated the Limitations section accordingly. The revised text
reads:
\end{response}

\begin{msquote}
PoolParty operates on sequences, not genomic loci or transcript models. It can
accept genomic coordinates as input to extract sequences, but does not
interpret transcript annotations or keep track of those coordinates through
subsequent design steps (e.g., as VaLiAnT does), making it less helpful for
genome-editing MAVE libraries that require this context throughout their
design.
\end{msquote}
...
\begin{response}
We have replaced ``lack'' with ``scarcity.'' The revised sentence reads:
\end{response}

\begin{msquote}
Designing these libraries, however, remains tedious and error-prone due to the
scarcity of purpose-built software.
\end{msquote}
...
\begin{response}
We have replaced ``assay-specific'' with more general wording and revised the
surrounding description. We also tested VaLiAnT
on noncoding regions and confirmed that it supports these designs. We have
revised the description as follows:
\end{response}

\begin{msquote}
Several other tools automate library design or sequence perturbation for
particular applications. For example, VaLiAnT \citep{Barbon2022th} generates
mutational scanning libraries of both coding and noncoding regions; supported
mutation types include single-nucleotide substitutions, amino acid
substitutions, and in-frame deletions.
\end{msquote}
...
\begin{response}
Indeed, most libraries designed for wet-lab experiments can be generated in
seconds to minutes on a standard CPU, as the reviewer notes. We have therefore
made the wording more specific:
\end{response}

\begin{msquote}
Importantly, no sequences are generated until the user requests them, so users
can explore and test multiple design options without generating each full
library.
\end{msquote}
...
\begin{response}
We are glad the reviewer found the three use cases well chosen; we selected
them to illustrate the types of library designs where we think PoolParty is
particularly useful. We appreciate the other positive feedback.
\end{response}
...
\begin{response}
We have retained these code panels because they show directly how the library
designs in panels A are specified using PoolParty. The examples are short and
also illustrate the relationship between the code and the corresponding DAGs
and outputs.
\end{response}
...
\begin{response}
\texttt{missense\_only\_first} selects the first codon listed for each target
amino acid in the specified codon table, which by default is ordered by human
codon-usage frequency. It does not count codons in the input sequence. We have
clarified this in the text:
\end{response}

\begin{msquote}
We note that \texttt{mutagenize\_orf} supports multiple codon mutation types;
this example uses \texttt{missense\_only\_first}, which selects the most
frequently used human codon for each target amino acid according to the
built-in codon-usage table.
\end{msquote}
...
\begin{response}
We have defined SpliceAI in the earlier Implementation subsection where the
model is first used. The revised text reads:
\end{response}

\begin{msquote}
SpliceAI \citep{Jaganathan2019ps} is a genomic deep learning model for
splice-site prediction in humans. We used the five-model ensemble to predict
the canonical 5'ss probability using $\pm$5~kb of genomic context (GRCh38).
\end{msquote}

\item Lien 238 ``varying strength'' -- maxentscan in legend, I would define this
is the amin body text too.

\begin{response}
We have clarified in the Results that cryptic splice-site strength refers to
the MaxEntScan score. The revised sentence reads:
\end{response}

\begin{msquote}
Using the 5'ss of SMN2 exon~7 as the canonical site, we used PoolParty to
construct a library in which cryptic 5'ss 9-mers of varying strength
(MaxEntScan score) were inserted at varying positions on either side of the
canonical 5'ss (see Implementation).
\end{msquote}

\item Lien 248 -- Fig.4G is introduced before 4E-F, perhaps generally introduce
Fig.4. then this could stay as is.

\begin{response}
We have moved the cross-validation result to the end of the paragraph so that
the Figure 4 panels are introduced in order. The revised paragraph reads:
\end{response}

\begin{msquote}
The GAMs closely reproduced the observed data (Fig.~\ref{ms-fig:figure4}B,C).
Cryptic sites on the exonic side suppressed canonical site usage more strongly
than those on the intronic side (Fig.~\ref{ms-fig:figure4}D), with the effect
peaking approximately 20~bp upstream of the canonical 5'ss. This is consistent
with the observation that cryptic 5'ss are preferentially depleted near
annotated ones on the exonic side \citep{Dawes2022empirical}. Stronger cryptic
sites drove greater suppression in both models (Fig.~\ref{ms-fig:figure4}E). The
effects of position and strength exhibited substantial interactions
(Fig.~\ref{ms-fig:figure4}F). Through a comparison to analogous linear models, we
observed that the non-linear terms and interaction terms in the GAMs
substantially improved predictive performance, with held-out $R^2$ of 0.82 for
exonic and 0.71 for intronic insertions (Fig.~\ref{ms-fig:figure4}G).
\end{msquote}
...
\begin{response}
As discussed in our response to Comment 1 above, we have revised both
statements:
\end{response}
...
\begin{response}
We have clarified that this refers to the SpliceAI surrogate modeling example
in the Results. The revised sentence reads:
\end{response}

\begin{msquote}
In the SpliceAI surrogate modeling example, the cryptic splice-site strength
and position recorded in each design card provided the covariates used for
surrogate modeling.
\end{msquote}
...
\begin{response}
We checked the current BMC policy and confirmed that this use is permitted when
disclosed. The software was developed through human coding and testing with
assistance from LLMs. LLMs were also used for copy-editing the manuscript,
while the scientific content and conclusions were determined and reviewed by
the authors.
\end{response}
