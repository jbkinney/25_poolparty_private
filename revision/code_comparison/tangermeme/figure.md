...
generated by `make_figure.py` from the same code listings that appear in the layout
below — edit the script, not the XML. See `comparison.md` for the verification and
the corrections made along the way, and `../README.md` for the shared drawing spec.

`make_figure.py` also checks that no two cells overlap, and the code in
`figure.drawio` has been diffed line for line against both the layout below and
`repro.py`.
...
│      cryptic_pool, positions=SCAN_POSITIONS,                 │  │      [one_hot_encode(site) for site in cryptic_sites])       │
