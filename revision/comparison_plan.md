...
| A2 | R1 #3, #5 | Feature table vs existing tools | Evidence complete; table not yet drafted |
...
`tool_survey/` holds, for 13 tools (12 competitors + PoolParty):

- `papers/` — 11 tool PDFs (SeqPro has no paper; software only)
- `extractions/`, `reviews/`, `final/` — 3-stage extract -> adversarial review -> fix
- `v2/` — re-scored against the v2 row set
- `verified/` — **8 argument-critical rows re-audited, one max-effort agent per
  row across all 13 tools**, primary sources only
- `ROWS_v3.md` — final row definitions
- `MATRIX_verified.md` — the verified matrix and the central result

**Central result:** PoolParty is the only tool scoring `yes` on all four
mutagenesis rows (`exhaustive_single_scans`, `sampled_random_mutagenesis`,
`higher_order_combinatorial`, `heterogeneous_components_one_library`). No other
tool reaches 3 of 4. It survived a verification pass that changed 25 cells and
demoted PoolParty on one row.

**Scope limit:** only those 8 rows are verified. The other 20 rows still carry
per-tool-agent values with known rater variance. Audit any row the same way before
putting it in the main-text table.
...
|---|---|---|
| T1 | Main-text feature table — general, not technical; Tier 2 in full, Tiers 1/3 collapsed; include a block of capabilities PoolParty lacks | choosing which rows; 20 rows still unverified |
