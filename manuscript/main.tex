...
PoolParty provides over 20 built-in Operations that users can choose from when constructing DAGs. Each Operation takes zero or more Pools as input and returns exactly one Pool as output. It is useful to think of Operations as falling into four functional categories (Fig.~\ref{fig:figure1}A): source, transformation, composition, and state Operations. \emph{Source Operations} create the initial Pools from which all downstream Pools are constructed. These may represent user-specified sequences, DNA sequences generated using position weight matrices (PWMs) or IUPAC motifs, random k-mers, barcodes, and so on. \emph{Transformation Operations} modify sequence content, e.g., generating sequence variants through point mutations, insertions, deletions, recombination, or shuffling. Specialized codon-aware Operations are provided for transformations that operate on open reading frames. \emph{Composition Operations} combine sequences from multiple parent Pools, either by concatenating parent sequences end-to-end (the \texttt{join} Operation) or by merging multiple Pools into one Pool (the \texttt{stack} Operation). \emph{State Operations} affect which sequences are in a Pool and how they are ordered, and include methods to select, reorder, filter, and replicate sequences.

\subsection{Sequence generation}

To generate a sequence library, PoolParty iterates through a series of \emph{states}, each of which is an integer that, together with a random seed, uniquely determines a sequence. The generation of each individual sequence proceeds in two steps, state assignment and sequence construction (Fig.~1B), a process that begins when the correpsonding sequence is passed to the root Pool. State assignment involves a \emph{backward pass} through the DAG: each Pool passes the state it recieves to its upstream Operation, and each Operation decomposes the state it recieves into an \emph{internal state} (which it keeps) and zero or more \emph{component states}. each of which is then passed to a distinct upstream Pool. Sequence construction then occurs during a \emph{forward pass} though the DAG: each Operation constructs a sequence based on its innternal state and the sequences recieved from the upstream Pools, then passes this sequence to the downstream Pool. The final constructed sequence is then recieved from the root Pool and recorded. 
...
\begin{itemize}
\item In \textbf{sequential mode}, the Operation exhaustively enumerates a finite set of sequence designs. For example, when \texttt{mutagenize} is instructed to generate single-nucleotide mutations in sequential mode, it yields one variant for each possible character change at each target position. In doing so, it interpretes its internal state as specifying which specific mutation to apply to which position.

\item In \textbf{random mode}, sequence designs are sampled stochastically. For example, the \texttt{shuffle} Operation (which always operates in random mode) randomly reordering the bases in a sequence. A master seed ensures that this process is reproducible. 
...
The way in which each Operation decomposes the state it recieves is designed to reverse the specific way that it constructs sequences during the forward pass. For most Operations, the output state is the Cartesian product of the Operation's internal state and the states of the input Pools. In such cases, the Operation uses mixed-radix decomposition to split the state into its component states. The \texttt{stack} Operation is different: its output states are the disjoint union of its input Pool states. The \texttt{stack} Operation therefore uses the state it is fed to select which input Pool it will use, and to instruct this Pool (but not the other Pools) which sequence to return. Keeping track of these rules across a complex DAG requires specialized bookkeeping. We therefore developed a back-end package called StateTracker that automates state composition and decomposition during forward and backward passes through the DAG.
...
\textbf{(A)} Library schematic showing TFBS placements across a 100-bp CRE in representative variants, with barcode assignments (24{,}000 sequences total).
...
\textbf{(C)} Styled \texttt{print\_library} output. TFBSs are colored by identity (HNF4A blue, PPARA purple, XBP1 orange); barcodes are shown in bold.}
\label{fig:figure3}
\end{figure*}
...
As a second example, we designed an MPRA library that tiles the binding sites of three liver-enriched TFs --- HNF4A, PPARA, and XBP1 --- across a 100~bp CRE while allowing both their relative order and orientation to vary (Fig.~\ref{fig:figure3}A,B). Following the oligo layout of \citet{Melnikov2012dw}, we define a template Pool containing an inert CRE region and a barcode region, both tagged with region markers (Section~2.5). Each TF is represented by a Pool containing its forward and reverse-complement binding sites. We then use \texttt{insertion\_multiscan} in random mode to insert these three Pools jointly at 1{,}000 sampled non-overlapping arrangements. For each sampled arrangement, PoolParty enumerates 8 orientation combinations ($2^3$ for three TFs), yielding 8{,}000 CRE variants. We then replicate each variant three times using \texttt{repeat} and fill the barcode region with unique barcodes from \texttt{get\_barcodes}, so that each CRE variant is paired with three distinct barcodes, giving a final library of 24{,}000 sequences (Fig.~\ref{fig:figure3}A). 
