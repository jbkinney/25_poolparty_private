...
\author[1]{Zhihan Liu\ORCID{0009-0004-9142-3777}}
\author[1]{Aidan Cordero\ORCID{0009-0007-2869-9721}}
\author[1,$\ast$]{Justin B. Kinney\ORCID{0000-0003-1897-3778}}

\authormark{Liu, Cordero, and Kinney}

\address[1]{\orgdiv{Simons Center for Quantitative Biology}, \orgname{Cold Spring Harbor Laboratory}, \orgaddress{\street{1 Bungtown Rd.}, Cold Spring Harbor, \state{New York}, \postcode{11724},  \country{United States}}}
...
\textbf{Availability and implementation:} PoolParty can be installed using the pip package manager and is compatible with Python $\geq$ 3.10. Documentation is provided at http://poolparty.readthedocs.io; source code is available at http://github.com/jbkinney/poolparty-statetracker.\\
...
Oligonucleotide pools have become essential tools in functional genomics. In massively parallel reporter assays (MPRAs), designed libraries of DNA sequences are routinely used to measure the regulatory activities of tens of thousands of candidate or variant regulatory elements in a single experiment \citep{Inoue2015cs}. In deep mutational scanning (DMS) experiments, coding sequences that have been either systematically or randomly mutagenized are commonly used to map protein fitness landscapes \citep{Fowler2014gq}. Both MPRAs and DMS experiments are examples of multiplexed assays of variant effect (MAVEs), a large and expanding family of high-throughput methods that quantitatively link sequence to function at scale \citep{Starita2017ec, Kinney2019jc}. The same idea extends beyond the wet lab: complex DNA sequence libraries are increasingly being used in silico to probe the behavior of genomic AI models \citep{Avsec2021bpnet, Toneyan2024creme, Seitz2024squid,Seitz2025, Nagai2026review} and to augment genomic AI training data \citep{Lee2023}.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure1_v3}
...
\begin{itemize}
\item In \textbf{sequential mode}, the Operation exhaustively enumerates a finite set of sequence designs. For example, when \texttt{mutagenize} is instructed to generate single-nucleotide mutations in sequential mode, it yields one variant for each possible character change at each target position. Its internal state specifies which mutation to apply at which position.

\item In \textbf{random mode}, the Operation samples sequence designs stochastically. For example, when \texttt{mutagenize} operates in random mode, it randomly selects which positions to mutate and which mutations to introduce. Its internal state, combined with a master seed, determines the specific random draws, ensuring reproducibility. 

\item In \textbf{fixed mode}, there is no internal state. Rather, output sequences are uniquely determined by the input sequences. One example is \texttt{join}, which concatenates its input sequences without modification.
\end{itemize}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure2_v2}
\caption{\textbf{A deep mutational scanning (DMS) library for protein GB1.}
\textbf{(A)} DAG and representative sequences for each of the four library components: all single amino acid substitutions, all double substitutions, random higher-order mutants, and replicats of the wild-type sequence. Mutated codons are highlighted with amino acid translations shown beneath.
...
Fig.\ \ref{fig:figure2}A illustrates the DAG used to construct this library; the code that establishes the DAG is shown in Fig.\ \ref{fig:figure2}B. First we use the \texttt{from\_seq} Operation to define a Pool named \texttt{orf\_pool} that represents the wild-type GB1 coding sequence. This Pool is then fed as input to four separate Operations, three of which are \texttt{mutagenize\_orf} and one of which is \texttt{repeat}. The first \texttt{mutagenize\_orf} operates in sequential mode and generates all 1,045 possible single amino acid mutations. The second also operates in sequential mode and generates all 546,085 double mutations. The third \texttt{mutagenize\_orf} operates in random mode, mutagenizing sequences stochastically at 10\% per position. The final Operation, repeat, creates 100 copies of the wild-type sequence. The four Pools that result from these operations are then merged by \texttt{stack} into the root Pool, which yields a total of 557,230 seequences. The design card provided with each variant (Fig.~\ref{fig:figure2}C) reports which Operation created it, the mutations (if any) it contains, and so on. Users can inspect the structure of the DAG by running the \texttt{print\_dag} method at the Python prompt (Fig.~\ref{fig:figure2}D).

\subsection{MPRA library for regulatory grammar}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figure3_v3}
...
MPRA libraries used to study regulatory grammar often vary the positions and orientations of transcription factor binding sites (TFBSs) \citep{Melnikov2012dw, Sharon2012io, GeorgakopoulosSoares2023tf} [MORE REFS]. For example, \citet{GeorgakopoulosSoares2023tf} tested over 200,000 sequences containing all pairwise and triplet arrangements of 18 TFBSs, in order to study the impact of ordder and orientation on gene expression. Designing such libraries requires substantially more complex combinatorics than standard DMS libraries do. PoolParty, however, handles this complexity without making additional demands on the user. 

As a second example, we designed an MPRA library that tiles the binding sites of three liver-enriched TFs---HNF4A, PPARA, and XBP1---across a 100~bp CRE while allowing both their relative order and orientation to vary (Fig.~\ref{fig:figure3}A,B). Following the oligo layout of \citet{Melnikov2012dw}, we start by creating a Pool representing a template sequence that contains a putatively inert CRE region (\texttt{<cre>}) and a barcode region (\texttt{<bc>}). Next we create a Pool representing the binding site for HNFA. This pool is then passed to the \texttt{flip} Operation, which causes iteration over the two possible orientations of this site (forward or reverse complement). Pools for PPARA and XBP1 are created similarly. The template Pool and three TFBS pools are then passed to the \texttt{insert\_multiscan} Operation, which creates 10,000 sequences by inserting the TFBSs at randomly chosen positions within the \texttt{<cre>} region of the template.  Each sequence is then replicated three times using the \texttt{repeat} operation. Finally, the \texttt{get\_barcodes} Operation is used to generate barcodes that are inserted into the \texttt{<bc>} region of the sequence. The result is a library of 30,000 sequences (Fig.~\ref{fig:figure3}A). 

Colored text is used to help visualize the architecture of the resulting sequences. After the template Pool is created, the \texttt{stylize} Operation is used to shade the \texttt{<cre>} region gray. Each TFBS Pool is also assigned a distinct color when it is created (HNF4A blue, PPARA purple, XBP1 orange), this time by passing the \texttt{style} parameter to \texttt{from\_seqs}. Barcodes are rendered in bold in a similar way (Fig.~\ref{fig:figure3}B). These styles are preserved as sequences pass through the DAG and rendered when the \texttt{print\_library} of the root Pool is called (Fig.~\ref{fig:figure3}C).
...
\begin{figure*}[p]
\centering
\includegraphics[width=0.85\textwidth]{figs/figure4_v3}

\caption{[CAPTION NEEDS AGGRESSIVE REWWRITE FOR CLARITY]
\textbf{Surrogate modeling of SpliceAI predictions using design-card features.}
...
Complex sequence libraries are increasingly being used to probe the regulatory mechanisms learned by genomic deep learning models, in silico using virtual assays \citep{Avsec2021bpnet, Toneyan2024creme, Seitz2024squid, Nagai2026review}. One way of doing this is called surrogate modeling. After scoring the sequences in a library with a model of interest, one infers a simpler (and more interpretable) model that predicts these scores based on specific sequence features. The desing cards supplied by PoolParty facilitates surrogate modeling by providing metadata describing how the sequence was constructed, thereby supplying covariates that can be used as input to surrogate models.

[THESE NEXT TWO PARAGRAPHS NEED AN AGGRESSIVE REWRITE FOR CLARITY]
For our third use case, we demonstrate this capability through a surrogate modeling study of the effects of cryptic 5' splice sites on the predictions of SpliceAI \citep{Jaganathan2019ps}. Using PoolParty, we designed a minigene library by varying the strength (MaxEntScan) of inserted cryptic donors and their position relative to the canonical 5'ss of SMN2 exon 7 (Methods). The library covered a full grid of 100 positions and 100 strength quantile bins, with design cards automatically recording these parameters for each variant (Fig.~\ref{fig:figure4}A). We paired each variant with a matched control and defined the cryptic donor effect as the difference in SpliceAI predictions at the canonical 5'ss between the two. Separate generalized additive models (GAMs) were fit to this quantity for exonic and intronic regions, using only smooth spline terms for strength, position, and their interaction (Fig.~\ref{fig:figure4}A).

The GAMs achieved a held-out $R^2$ of 0.82 (exon) and 0.71 (intron), and their response surfaces matched the observed data (Fig.~\ref{fig:figure4}B,C). Positional effects showed stronger suppression for exonic than intronic cryptic sites (Fig.~\ref{fig:figure4}D), with effects peaking at ~20 bp upstream, consistent with a prior report of preferential depletion of decoy donors proximal to annotated 5'sss on the exonic side \citep{Dawes2022empirical}. In terms of strength effects, stronger cryptic donors drove larger suppression in both models (Fig.~\ref{fig:figure4}E). Positional and strength effects also interacted in a non-additive manner, as shown by the interaction surface (Fig.~\ref{fig:figure4}F). We compared the GAMs to simpler surrogate alternatives and found that incorporating non-linearity and interaction terms enhanced predictive performance (Fig.~\ref{fig:figure4}G).
...
We have presented PoolParty, a Python package that replaces the ad hoc scripting commonly used to design oligonucleotide libraries with a declarative, composable, and flexible framework. We demonstrated PoolParty by considering three use cases: library design for a DMS experiment on protein GB1; library design for an MPRA probing transcriptional regulatory grammar; and a surrogate modeling analysis of the effects that cryptic donor sites have on the alternative splicing predictions of SpliceAI. These examples illustrate the flexibility of PoolParty and its applicability to both experimental and computational studies.

We wish to highlight the design card that PoolParty returns with each generated sequence. By providing a structured record of how each sequence was constructed, PoolParty facilitates the use of sequence design parameters as covariates in downstream analysis. In the SpliceAI example, design cards enabled surrogate modeling without any post hoc sequence parsing; we expect similar strategies will be useful for systematically interrogating other genomic AI models.

PoolParty was designed to address a focused computational problem: provide a lightweight tool for streamlining the design of DNA sequence libraries. PoolParty is not intended as a bona fide sequence optimization tool, e.g., for designing synthetic proteins or cis-regulatory elements with specified functional properties. We also note that the current implementation generates sequences one at a time, requiring roughly one millisecond per sequence on a standard laptop. This suffices for typical oligonucleotide libraries but has  the potential to bottleneck large-scale tasks; a future implementation of batch-mode sequence generation would address this.

We anticipate that PoolParty will substantially ease the design of MPRAs and DMS experiments, as well as other high-throughput functional genomics assays. And as genomic AI models become increasingly accurate, PoolParty will help accelerate efforts to gain insight into the molecular mechanisms that underlie the predictions of these models. 
