...
def validate_iteration_order(iteration_order: Sequence[int], n: int) -> None:
    ...
    if len(iteration_order) != n:
        raise ValueError(
            f"iteration_order must have length {n} (number of items), got {len(iteration_order)}"
        )
    expected = set(range(n))
    actual = set(iteration_order)
    if actual != expected:
        raise ValueError(
            f"iteration_order must contain exactly the integers 0 to {n - 1}, got {sorted(actual)}"
        )
