...
from poolparty.types import Callable, Sequence
...
def validate_single_region_from_list(
    regions: Sequence[ParsedRegion], name: str, seq: str = ""
) -> ParsedRegion:
    ...
    if len(matching) == 0:
        ...
        if available:
            raise ValueError(
                f"Region '{name}' not found in sequence. Available regions: {available}"
            )
        else:
...
def nontag_pos_to_literal_pos(seq: str, nontag_pos: int) -> int:
    ...
    if nontag_pos < 0 or nontag_pos > seq_len:
        raise ValueError(f"nontag_pos ({nontag_pos}) out of range [0, {seq_len}]")
